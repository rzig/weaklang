...
void Lexer::lex(const std::string &to_lex) {
    ...
    while (current < to_lex.size()) {
        ...
        switch (start_token_c) {
            ...
            case '!': tokens.push_back(Token(EXCLA, to_lex.substr(start, 1), making_token_line, making_token_col));break;
            case '=': tokens.push_back(Token(EQUALS, to_lex.substr(start, 1), making_token_line, making_token_col));break;
            case '>': tokens.push_back(Token(GREATER, to_lex.substr(start, 1), making_token_line, making_token_col));break;
            case '<': tokens.push_back(Token(LESSER, to_lex.substr(start, 1), making_token_line, making_token_col));break;
        }
    }
